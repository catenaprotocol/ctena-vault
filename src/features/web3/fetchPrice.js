...
const fetchTokens = async () => {
  ...
  try {
    const response = await axios.get(`https://api.ctena.finance/prices?_=${cacheBuster}`);
...
const fetchLPs = async () => {
  ...
  try {
    const response = await axios.get(`https://api.ctena.finance/lps?_=${cacheBuster}`);
    return response.data;
  } catch (err) {
    console.error(err);
    return {};
...
export function initializePriceCache() {
  ...
  oracleToIds.get('tokens').push('CTENA');
